...
\begin{itemize}
    \item \textbf{Median path}: Pass zero shocks ($\varepsilon^r = \varepsilon^s = 0$ for all $t$). Produces the expected-value trajectory.
...
\begin{enumerate}
    ...
    \item Compute dynamic consumption rate $c_t$ using realized weights
    \item Compute consumption $C_t$ and apply constraints
    \item Evolve wealth: $FW_{t+1} = FW_t(1 + R^p_{t+1}) + Y_t - C_t$
\end{enumerate}
