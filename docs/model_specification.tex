...
Assuming HC has stock exposure $\beta_{HC}$ and the rest is bond-like:
\begin{align}
HC^{\text{stock}} &= \beta_{HC} \cdot HC_t \\
HC^{\text{bond}} &= (1-\beta_{HC}) \cdot HC_t \cdot \frac{D^{HC}_t}{D} \\
HC^{\text{cash}} &= (1-\beta_{HC}) \cdot HC_t \cdot \left(1 - \frac{D^{HC}_t}{D}\right)
\end{align}
where $D$ = duration of bonds in the financial portfolio. The ratio $D^{HC}_t / D$ determines how much of the non-stock HC is ``bond-equivalent'' vs ``cash-equivalent'' based on relative durations.
...
\subsection{Jensen's Correction: Zero-Shock Path vs Monte Carlo}
\label{sec:jensen}

The consumption rate includes Jensen's correction (Eq.~\ref{eq:consumption_rate}):
\[
c_t = \underbrace{\mathbb{E}_t[R^p_{t+1}]}_{\text{arithmetic mean}} - \tfrac{1}{2}\text{Var}_t(R^p_{t+1}) + \delta
\]
But wealth evolves at the \emph{realized} portfolio return, which differs between the two modes:

\textbf{Zero-shock path.} With $\varepsilon^r = \varepsilon^s = 0$:
\begin{itemize}
    \item Each asset earns its \emph{arithmetic} mean: $R^s = r + \mu_s$, $R^b = r + \mu_b$, $R^{\text{cash}} = r$
    \item The portfolio return used in wealth evolution is $R^p = \mathbb{E}[R^p]$ (arithmetic mean)
    \item The consumption rate uses $\mathbb{E}[R^p] - \tfrac{1}{2}\text{Var}(R^p)$ (geometric mean)
    \item The wedge $\tfrac{1}{2}\text{Var}(R^p)$ means wealth grows \emph{faster} than consumption depletes it
\end{itemize}
This makes the zero-shock path an \textbf{expected-value (mean) trajectory}, not a true median. It is the path wealth would follow if every asset earned its arithmetic mean return every period.

\textbf{Monte Carlo.} With random shocks:
\begin{itemize}
    \item Each asset earns a realized return that includes shocks
    \item The \emph{median} compound return across paths $\approx$ geometric mean $= \mathbb{E}[R^p] - \tfrac{1}{2}\text{Var}(R^p)$
    \item The consumption rate and median wealth growth are approximately aligned
\end{itemize}
The median of the Monte Carlo wealth distribution will therefore lie \emph{below} the zero-shock path. The gap is approximately $\tfrac{1}{2}\text{Var}(R^p)$ per period, compounded---reflecting the standard result that $\mathbb{E}[W] > \text{median}(W)$ for log-normally distributed wealth.
...
\begin{enumerate}
    ...
    \item Evolve wealth: $FW_{t+1} = FW_t(1 + R^p_{t+1}) + Y_t - C_t$ using \emph{realized} $R^p_{t+1}$
\end{enumerate}
