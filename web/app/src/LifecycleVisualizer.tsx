...
function StepperInput({ label, value, onChange, min, max, step, suffix = '', decimals = 0 }: StepperInputProps) {
  ...
  const inputValueRef = React.useRef(value.toFixed(decimals));

  const setInputValueBoth = (v: string) => {
  ...
  const handleBlur = () => {
    ...
    if (!isNaN(parsed)) {
      ...
      onChange(clamped);
      setInputValueBoth(clamped.toFixed(decimals));
    } else {
      setInputValueBoth(value.toFixed(decimals));
    }
  };
  ...
  const handleStep = (delta: number) => {
    ...
    onChange(newValue);
    setInputValueBoth(newValue.toFixed(decimals));
  };
    ...
    <div style={{ marginBottom: '8px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
        ...
        <input
          type="text"
          value={inputValue}
          onChange={(e) => setInputValueBoth(e.target.value)}
          onBlur={handleBlur}
